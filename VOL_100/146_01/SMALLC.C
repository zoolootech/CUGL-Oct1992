...
VERSION:      Ver 1.1;
DESCRIPTION:  "Small c compiler for 6800 based on Ron Cain's Small
               c for 8080";
KEYWORDS:     Small c, compiler, FLEX;
SYSTEM:       FLEX ver 2.1;
FILENAME:     Small c.c;
AUTHORS:      Ron Cain, Serge Stepanoff;
*/
/************************************************/
/*                                              */
/*              small-c compiler                */
...
constant(val)
        int val[];
{       if (number(val)|pstr(val))
        {
                pseudoins(0); defword(); }
        else if (qstr(val))
        {       pseudoins(0); defword(); printlabel(litlab);
                outbyte('+');
                }
        else return (0);        
        outdec(val[0]);
