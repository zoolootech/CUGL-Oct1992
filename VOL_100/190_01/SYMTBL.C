...
{
	register int result;
	struct _mtable *bottom = &mtable[0];
	struct _mtable *top = &mtable[MTSIZE - 1];
	struct _mtable *middle;
...
char *symbol;
{
	...
	register int result;
	register int bump = 0;					/* store index 'bump' factor */
	struct _symbol *bottom;
	struct _symbol *top;
	struct _symbol *middle;
...
ndex to index + 1 */
		(symbols - index + 1) * sizeof(struct _symbol));	/*