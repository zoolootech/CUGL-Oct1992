...
/* From DECUS C Tools package on DEC systems - for non-commercial
    use only.  Modifications by Chuck Allison, April-October 1985:

        - handles quoted arguments (for embedded spaces)
        - distinguishes case (ignore with -i)
        - list filename only option (-l)
*/
...
char *s;
{
   ...
   fprintf(stderr,
      "Usage: grep [-cfilnv] pattern [file ...]\n");
   exit(1);
}
...
char       *fn;       /* File name (for -f option)  */
{
   ...
   {
      if (fflag && fn)
            printf("%-40.40s: ",trim_fspec(fn,40));
      printf("%6d\n", count);
   }
}
