...
int termflag;		/* if true, keep terminal in normal mode */

#if	CALLABLE
int exitflag;		/* set when time to exit */
...
/* free all dynamically allocated areas */
freeall()
{
	kdelete();
	frallwindows();
...
char    *argv[];
{
	...
	BUFFER *firstbp;		/* ptr to first buffer in cmd line */
	int basec;			/* c stripped of meta character */
	int viewflag;			/* are we starting in view mode? */
        int gotoflag;                   /* do we need to goto a line at start? */
        int gline;                      /* if so, what line? */
        int searchflag;                 /* Do we need to search at start? */
	int savelflag;			/* temp store for lastflag */
...
#if	CRYPT
	cryptflg = FALSE;	/* no encryption by default */
#endif
		...
		if (argv[carg][0] == '-') {
			switch (argv[carg][1]) {
				...
				case 'G':
					...
					break;
				case 'i':	/* -i<var> <value> */
				case 'I':	/* set inital value for var */
					bytecopy(vname, &argv[carg][2], NVSIZE);
					findvar(vname, &vd, NVSIZE + 1);
					++carg;
					if (vd.v_type == -1)
				mlwrite("%%No such variable as '%s'", vname);
					else if (carg >= argc) svar(&vd, "");
					else svar(&vd, argv[carg]);
					break;
#if	CRYPT
				case 'k':	/* -k<key> for code key */
				case 'K':
					cryptflg = TRUE;
					bytecopy(ekey, &argv[carg][2], NPAT-1);
					break;
#endif
				...
				case 'S':
					searchflag = TRUE;
					bytecopy(pat,&argv[carg][2],NPAT-1);
					setjtable(pat);
					break;
				case 't':
				case 'T':
			...
			if (cryptflg) {
				...
				crypt(ekey, (unsigned) strlen(ekey));
				bytecopy(bp->b_key, ekey, NPAT-1);
			}
#endif
		}
	}
...
#if	CALLABLE
	if (exitflag == TRUE) return(GOOD);
#endif

	/* execute the "command" macro...normally null */
	savelflag = lastflag;	/* preserve lastflag through this */
	execute(FUNC|'C', HOOK, 1);
	lastflag = savelflag;
...
int useoldargs = FALSE;	/* saved universal arg so C-U n meta command */
int execnold;		/* repeats command instead of meta */
int execfold;
int inmeta = FALSE;	/* true if next command was prefixed by a meta */
int incex = FALSE;	/* true if next command was prefixed by a C-X */
int inspecial = FALSE;	/* true if next command was prefixed by a FN */

meta(f, n)	/* dummy function for binding to meta prefix */
{
	inmeta = TRUE;
	useoldargs = TRUE;
	execfold = f;
...
cex(f, n)	/* dummy function for binding to control-x prefix */
{
	incex = TRUE;
	useoldargs = TRUE;
	execfold = f;
...
#if	DECEDT
special(f, n)	/* dummy function for binding to special prefix */
{
	inspecial = TRUE;
	useoldargs = TRUE;
	execfold = f;
...
execute(c, f, n)
{
	...
	if (f == HOOK)
		f = FALSE;
	else if (useoldargs == TRUE) {
		if (f == FALSE) {f = execfold; n = execnold;}
		useoldargs = FALSE;
	}
...
#if	DECUSC
fgetc(iop) FILE *iop; { return(getc(iop)); }
fputc(c, iop) char c; FILE *iop; { return(putc(c, iop)); }
#endif
...
{
	unsigned *mp;	/* ptr returned from malloc */
	char *malloc();

	mp = malloc(nbytes + sizeof(unsigned));
	if (mp != NULL) {
		envram += nbytes;
		*mp = nbytes;
		++mp;
