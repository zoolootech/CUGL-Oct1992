...
#endif
#if C86
	if (ERROR == (iobuf = fopen( filename,"r")))
#endif
#if DeSmet
	if (FERR == (iobuf = fopen( filename,"r")))
#endif
...
#endif
if (debug && c == EOF) fprintf(stderr, "fgets2: EOF\n");
	*cptr = '\0';
	return s;
}
/**************************************************************
...
switch (c_type)
	...
	case ST :	/* stop(pause) at each page?*/
		set(&PAGESTOP,arg_val,(char)'0',YES,NO,YES);
		break;
	...
	case FF :	/*formfeed*/
		set(&FFEED,arg_val,(char)'0',FF_DEF,NO,YES);
		break;

	case SC :	/*space character*/
		if(arg_typ) arg_val=arg_typ;
		setS(&SCVAL,arg_val,(char)'0',SC_INI,BLANK,127);
		break;

	case OW :	/*output device width*/
		setS(&OWVAL,arg_val,(char)'0',OW_INI,RMVAL,HUGE);
		break;
	case TS :	/*tabsize*/
		setS(&TABSIZ,arg_val,(char)'0',TS_DEF,1,HUGE);
		break;
	...
	case DB :	/*debug*/
		setS(&debug,arg_val,(char)'0',NO,NO,YES);
	...
	case TC :	/*translate character flag*/
		if(arg_typ) arg_val=arg_typ;
		setS(&TCVAL,arg_val,(char)'0',TC_DEF,BLANK+1,127);
		break;

	case CF :	/*translate character flag*/
		if(arg_typ) arg_val=arg_typ;
		setS(&CFVAL,arg_val,(char)'0',CF_DEF,BLANK+1,127);
		break;

	case IC :	/*insert character for macro replace*/
		if(arg_typ) arg_val=arg_typ;
		setS(&ICVAL,arg_val,(char)'0',IC_DEF,BLANK+1,127);
		break;
