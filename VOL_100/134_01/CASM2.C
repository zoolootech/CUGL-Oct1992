...
process_line()
{
	...
	if (op) {
	   ...
	   if (streq(op,"FUNCTION")) {
		...
		nfcount = 0;
		pastnfs = 0;
		condptr.cb = &condbuf; *condptr.ci = 0x7FFF;
	...
	if (doingfunc && (streq (op, "IF") || streq (op, "ENDIF"))) {
		if (!pastnfs) flushnfs ();
		*condptr.ci++ = relblc;
		strcpy (condptr.cb, linbuf);
		condptr.cb += strlen (condptr.cb) + 1;
		*condptr.ci = 0x7FFF;
		return (fputnam (obuf, linbuf));
		}
	...
	if (!doingfunc || (!label && !op))	/* if nothing interesting on */
		return fputnam(obuf,linbuf);	/* line, ignore it	*/
...
out:
	if (!op) return fputnam (obuf, linbuf);	/* if label only, all done   */

						/* if a non-relocatable op,  */
	if (norelop(op)) return fputnam(obuf,linbuf);	/* then we're done   */
...
doreloc()
{
	...
	for(i = 0; i < relblc; i++)
		...
		while (*condptr.ci <= i) {
			++condptr.ci;
			fputnam (obuf, condptr.cb);
			condptr.cb += strlen (condptr.cb) + 1;
			}
		sprintf(undrbuf,"\t\tDW\t%s$R%03d\n",cfunam,i);
		fputnam (obuf, undrbuf);
		}
	while (*condptr.ci != 0x7FFF) {
		++condptr.ci;
		fputnam (obuf, condptr.cb);
		condptr.cb += strlen (condptr.cb) + 1;
		}
