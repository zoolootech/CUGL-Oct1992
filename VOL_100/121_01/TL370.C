	...
	if (miready()) {
	  ...
	  if (tflag) {
		if(c == ETX) {
			...
			if(c == XON) pflag = FALSE ;
			else if(c == XOFF) pflag = TRUE ;
			}
		}
...
special()
{
		...
		case 'V':  if (rflag) {
			   ...
			   else printf("No receiving file open");
			   goto lf;
...
askstuff()
{
	...
	fflag = shflag = ask("Do you want Ascii handshaking") ;
	scount = 0;
...
reset()
{
	timoutf = rflag = tflag = shflag = chflag = cflag = 0;
	scount = 0;
	spflag = 1;
	dodflag = !hdflag;
	didflag = 1;
...
xmit()
{
	...
	lastc = c;
	if(shflag && (c == '\n')) pflag = TRUE ;
	/* I need to have control over this later on... */
	if(c == 0x1a) return TRUE ;
	if (dodflag) display(c);
	if (bcount != SECSIZ) return FALSE ;
	bcount = 0;
