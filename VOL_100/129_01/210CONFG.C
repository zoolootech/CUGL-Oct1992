...
/*				History 				*/
/*									*/
/* 84Dec28 HAW	Now Configure identifies itself.			*/
...
main() {
	...
	if (args = sscanf(line, "%s %s %x ", cmd, var, &arg)) {
		...
		printf("#define '%s' as %x\n", var, arg);
		    ...
		    noMail	= !arg;
		} else if (strCmp(var, "CPU8085"   )	== SAMESTRING) {
		    secondsfactor = (arg ? 40 : 55);
		} else {
