...

{
	...
	balp = "()";
	if (argc < 2) {
		fprintf(STDERR, "Usage: m4 filename");
		exit();
	}
	if (fopen(argv[1], inbuf) == ERROR) {
...

{
	...
	p = &deftbl[tcnt];
	if ((p->name = alloc(strlen(s)+1)) == NULL) {
		fprintf(STDERR, "Variable table overflow!\n");
		exit();
	}
	strcpy(p->name, s);
	if ((p->repn = alloc(strlen(defn)+1)) == NULL) {
		fprintf(STDERR, "Variable table overflow!\n");
		exit();
	}
	strcpy(p->repn, defn);
...
{
	if (argp > ARGSIZE) {
		printf(STDERR, " Arg stack overflow!\n");
		exit();
	}
	argstk[argp] = evlp;
	return(argp+1);
...

{
	if (!cp)
		...
		if (ep > EVALSIZE) {
			fprintf(STDERR, "Evalation stack overflow!\n");
			exit();
		}
		evalst[ep++] = c;
	}
