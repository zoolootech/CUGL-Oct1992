...

{
	...
	/* evaluate it */
	if ((sp = getval(buffer)) == NULL) return(FALSE);
	strcpy(buffer, sp);
	return(TRUE);
}
...

{
	...
	bp = getdefb();
	bp = tenexbuf(TRUE, "Execute buffer", bp ? bp->b_bname: "main");
#else
        char            bufn[NBUFN];

	/* find out what buffer the user wants to execute */
        if ((s = mlreply("Execute buffer: ", bufn, NBUFN)) != TRUE)
		return (s);
