...
usebuffer(f, n)
{
	...
	bp = getdefb();
	bp = tenexbuf(TRUE, "Use buffer", bp ? bp->b_bname: "main");
	if (!bp)
		return(ABORT);
#else
        register int    s;
...
prevbuffer(f, n)	/* switch to the previous buffer in the buffer list */
...

{
	...
	if (curbp->b_active != TRUE) {		/* buffer not active yet*/
		...
		readin(curbp->b_fname, TRUE, FALSE);
...

{
	...
	/* make a completion for a buffer name */
	bp = getdefb();
	bp = tenexbuf(TRUE, "Kill buffer", bp ? bp->b_bname: "main");
	if (!bp) return(ABORT);
#else
        register int    s;
        char bufn[NBUFN];
...

{
	...
	register int	i;
        ...
        if (addline("ACTN MODES         Size Buffer        File") == FALSE
        ||  addline("---- -----         ---- ------        ----") == FALSE)
                return (FALSE);
		...
		for (i = 0; i < NUMMODES; i++) {
			if (bp->b_mode & (1 << i))
                ...
                *cp1++ = ' ';                   /* Gap.                 */
                ...
                if (*cp2 != 0) {
                        while (cp1 < &line[4+1+5+1+6+1+NBUFN])
                                *cp1++ = ' ';
                        while ((c = *cp2++) != 0) {
                                if (cp1 < &line[128-1])
                                        *cp1++ = c;
