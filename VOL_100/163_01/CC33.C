...
constant(lval) int lval[]; {
  ...
  *lval=1;     /* assume it will be a constant */
  if(number(++lval))   immed();
    ...
    *(lval-1)=0;  /* nope, its a string address  */
    addr2();
    printlabel(litlab);
    outbyte('+');
    }
...
number(val) int val[]; {
  ...
  if(ch == '0') { /* if octal or hex */
    gch();
    if(ch == 'X' || ch == 'x') { /* if hex */
      gch();
      while(1)
        if(numeric(ch)) k = (k << 4) + (inbyte()-'0');
        else if (ch >= 'a' && ch <= 'f') k = (k << 4) + (inbyte()-'a'+10);
        else if (ch >= 'A' && ch <= 'F') k = (k << 4) + (inbyte()-'A'+10);
        else break;
      }
    else {
      while(ch >= '0' && ch <= '7') k = (k << 3) + (inbyte()-'0');
      }
    }
  else {
...
stowlit(value, size) int value, size; {
  if((litptr+size) >= LITMAX) {
    error("literal queue overflow"); exit(EUSER);
    }
