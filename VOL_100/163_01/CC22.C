ifline() {
  ...
  while(1) {
    ...
    if(match("#ifdef")) {
      ...
      blanks();
      if(symname(mname, NO)==0) {
        illname();
        kill();
        continue;
        }
#ifdef HASH
      if(search(mname, macm, NAMESIZE+2, MACNEND, MACNBR, 0)==0)
#else
      if(findmac(mname)==0)
#endif
        skiplevel=iflevel;
...
#ifdef HASH
      if(search(mname, macn, NAMESIZE+2, MACNEND, MACNBR, 0))
#else
      if(findmac(mname))
#endif
        skiplevel=iflevel;
    ...
    if(skiplevel) continue;
    if(ctext) { /* if interleaving source */
      /* interleave only non-blank lines */
      ptr = line;
      while(*ptr) if(*ptr == ' ') ++ptr; else break;
      if(*ptr) { /* if non-blank */
        comment();
        lout(line, output);
        }
...
addmac() {
  ...
  while(putmac(gch()));
  if(macptr>=MACMAX) {
    error("macro string queue full"); exit(EUSER);
    }
  }
...
clearstage(before, start) char *before, *start; {
  *stagenext=0; /* terminate current character string */
  ...
  if(start) { /* if code is not to be flushed, output it --
                                  note that this will only be done when
                                  exiting outermost expression, and thus,
                                  start addresses the front of the buffer */
    peephole(start);
    }
  }
...
outdec(number)  int number; {
  ...
  if(number < 0) { /* must have been -32768 */
    outstr("32768");
    }
  else while (k>=1) {
    c=number/k + '0';
