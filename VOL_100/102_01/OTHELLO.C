...
*** Features added by Gilbert Shapiro:

(1) A view of the board after the player has moved but before the
computer's response. (2) When player responds to "Move:" by <CR>,
provides a list of available options on the screen. (3)
Displays a board with "X" in all legal positions when player's move is
illegal and numeric.  And (4) substitutes a reverse video * for @ as
the white piece, if terminal is an H19.

The program uses #ifdef wherever required, so that the original
OTHELLO can be reinstated by deleting or commenting out the line
"#define H19."  All new functions are at the end of the file. The
revised program works on an H89.  All screen formatting is performed
by direct cursor addressing to ease portability. (Heath direct cursor
addressing is "033Y" followed by an indication of the row and column.)
The functions of other Heath screen manipulation commands are
indicated in comments, to facilitate rewriting for other equipment.
--GS  ***
...
#define H19     	/* GS -- Deletion of this line reverts program to 
			   original features. */
#define BLACK '*'
#define WHITE '@'
#define EMPTY '-'
...
dispopts()		/* GS -- Displays options, after <CR>.  Works on H89.
				Uses only direct cursor addressing. */
				
{	char row, col;
	int i;
	clrq3();
	row = '\053';
	printf("\033Y%c\107g:  Computer plays both sides\n",row++);
	printf("\033Y%c\107a:  Analysis of your possible moves\n",row++);
	printf("\033Y%c\107hn: Sets handicap\n",row++);
	printf("\033Y%c\107f:  Forfeit current move\n",row++);
	printf("\033Y%c\107q:  Quit current game\n",row++);
	printf("\033Y%c\107b:  Print board again\n",row++);
	printf("\033Y%c\107s:  Print score, tell who's winning\n",row++);
	printf("\033Y\066\047");
}

displegl(b,p,o,e)		/* GS --  For H89, Displays legal moves as X */
char b[8][8], p,o,e;		/* Based on analyze() */
{
	struct mt  t[64];
	char a[8][8];
	int i,j,k,c;
	...
	for (i=1; i<=8; i++) for (j=1; j<=8; j++)
		a[i][j] = (a[i][j] >= 'A' && a[i][j] <= 'Z') ? 'X' : a[i][j];
	prtbrd_rt(a);
}

clrq3()			/* GS -- Clears the lower right quadrant of screen */
{
	int i;
	char row, col;
	col = 71;	/* For H89, this is screen col. 40 */
	printf("\033x5");	/* For H89, shuts off cursor. */
	for (row=42; row<=55; row++) {     /* Screen rows 11 to 24 */
        	printf("\033Y%c%c", row, col);
		for (i=1; i<=41; i++) putchar(' ');
	}
	printf("\033y5");	/* Turns on cursor */
}

prtbrd_rt(b)		/* GS - prtbrd modified to print on the right
				half of the screen. */
char b[8][8];
{
	char row, col;
	...
	clrq3();
	row = '\053' - mefirst;
	printf("\033Y%c\107   1 2 3 4 5 6 7 8\n",row);
	for (i=0; i<8; i++) {
		row = '\054' + i - mefirst;
		printf("\033Y%c\107",row);
		printf("%2d",i+1);
		for (j=0; j<8; j++) {
			putchar(' ');
...
clrq3()			/* GS -- Clears the lower right quadrant