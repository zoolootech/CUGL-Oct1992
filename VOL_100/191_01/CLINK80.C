/* CLINK/80 v. 6-9-85 compile with -k switch         */
/* Linkage of C/80 source code subroutines           */
/* Phillip L. Emerson   Cleveland State University   */
/* first version, 4-24-84                            */
/* modified 9-17-84 for some filenames in output     */
/* modified 2-20-85 for single pass if keyfile exists*/
/* modified 6-7-85: took out #define DONOTHING, put in
   #define LF,SP; simplified a few routines; hashlist
   structures now used for both data bases -- library
   function intercalls (table1) and program function
   calls netted up (table2). Also, routine netup() was
   modified to erase its tracks to prevent endless
   cycling (stack overflow) with cycling function calls */
...
#define VOID -1
#define NULL 0
#define UNDRSC '\137'   /*assumes ASCII*/
#define BELL '\007'
#define APOSTR '\047'
#define QUOTE '\042'
#define BACKSL '\134'
#define LF '\n'
#define SP '\40'
#define BUFSIZ 5000      /*must hold any lib. func.*/
...
#define alloc sbrk       /*a C/80 optional primitive alloc */
char progfil[FNAMSZ],libfil[FNAMSZ],keyfil[FNAMSZ],outfil[FNAMSZ];
char *fubuff, *linebuf ;
int prchan,lichan,keychan,ouchan,quotes,apostrs,combal;
...
char *strsave(s) char *s; {char *p ;
   if (((p=alloc(strlen(s)+1)) == VOID) || (p == NULL))
          return(NULL) ;
   strcpy(p,s); return(p) ; }
...
{  putstr("CLINK/80 v. 6-9-85\n") ;
  linebuf = alloc(LINSIZ) ;
  table1 = (struct list **) alloc(HASHSIZ*(sizeof(struct list *))) ;
  table2 = (struct list **) alloc(HASHSIZ*(sizeof(struct list *))) ;
  tabinit() ;
  fubuff = alloc(BUFSIZ) ;
...
struct list *insert(name,def,table) char *name,*def ;  struct list **table;
  {struct list *n; int val;
   ...
   n = (struct list *) alloc(sizeof(struct list)) ;
...
char *getfrst(byte,staddr) char byte,*staddr;
  { char *q;
    for (q=staddr; ( (*q=gignor()) != VOID) && ((*q != byte)
         || quotes || apostrs) && (q < (fubuff+BUFSIZ-MARGA)) ;
                 q++)  ;
    if (*q == VOID) return(VOID) ;
    if (q > (fubuff+BUFSIZ-MARGB)) {putstr("buf ovrfl\n"); exit() ; }
    *++q = NULL ; return(q) ;
  ...
  {struct list *j ;
   ...
   while( (getl0(linebuf,keychan) != VOID) )
     {getl0(fubuff,keychan) ;
      if( (j=insert(linebuff,fubuff,table1)) == NULL ) return(VOID) ;
