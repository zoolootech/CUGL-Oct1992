/* TLCPROCF.C - "The Last Cross-referencer" - C Process Files routines	*/
...
static	line_list_type	* prev_id_line_ptr;
static	char			prev_tok_string[80];
...
static VOID print_listing_line()
	...
	if (ok_to_print() && parm_rec.do_listing)
		{
...
debug(puts("  next linerec on list");)
	...
	/* FIFO(1) - save previous line reference, for reftype update of FUNCHEAD */
	prev_id_line_ptr = new_line_ptr;
...
boolean * found_include;
	...
	do	{
		token.tok_string[79] = 0;
		strcpy(prev_tok_string, token.tok_string);
		error = get_token();
		if (error)
...
/*
debug(printf("parseLine():blklvl=%d token=%d'%s'\n",\
block_level,token.tok_type,token.tok_string);)
*/
			if (token.tok_type=='#' && token.tok_column==1)
				{
				*found_include = do_directives();
				saw_id = FALSE;
				}
			else
				{ /* parse anything else normally */
				switch (token.tok_type)
					...
					case TOK_ID:
							...
							if (is_resword(token.tok_string))
								...
								saw_id = TRUE;
/*debug(puts("  SAW_ID=TRUE!");)*/
								if (block_level==0 &&
									strcmp(prev_tok_string,"define")==0)
									{
/*debug(printf(" prev=DEFINE\n");)*/
									ref_kind = REF_DEFINE;
									}
								else
									ref_kind = REF_BLANK;
					...
					case TOK_LPAREN:
						if (block_level==0 && prev_id_line_ptr!=NULL && saw_id)
							{
/*
debug(printf(" prev=FUNCHEAD linerecptr=%06lx\n",prev_id_line_ptr);)
*/
							/* just came upon "id(", if not DEFINE, its a fn() */
							if (prev_id_line_ptr->reference_type == REF_BLANK)
								prev_id_line_ptr->reference_type = REF_FUNCHEAD;
							}
						saw_id = FALSE;
						break;
...
debug(printf("process_a_file:<%d>'%s'\n",file_rec.curr_file->num_lines,curr_line););
				...
				/* don't parse if just listing (but need to later!) */
				if (parm_rec.do_xref)
...
/* NOTE: This should be called outside the loop eventually.. but resetting
	the scanner after every file clears up any trailing commente-out chunks */
