...
struct amode    *makedreg(r) /* make an address reference to a data register. */
int     r;
{       struct amode    *ap;
...
struct amode    *makeareg(r) /* make an address reference to an address register. */
int     r;
{       struct amode    *ap;
        ap = xalloc(sizeof(struct amode));
        ap->mode = am_areg;
...
struct amode    *make_mask(mask) /* generate the mask address structure. */
int     mask;
{       struct amode    *ap;
        ap = xalloc(sizeof(struct amode));
        ap->mode = am_mask;
...
struct amode    *make_direct(i) /* make a direct reference to an immediate value. */
int     i;
{       return make_offset(makenode(en_icon,i,0));
}

struct amode    *make_strlab(s) /* generate a direct reference to a string label. */
char    *s;
{       struct amode    *ap;
        ap = xalloc(sizeof(struct amode));
        ap->mode = am_direct;
...
genwhile(stmt) /* generate code to evaluate a while statement. */
struct snode    *stmt;
{       int     lab1, lab2;
...
gen_for(stmt) /* generate code to evaluate a for loop */
struct snode    *stmt;
...
genif(stmt) /* generate code to evaluate an if statement. */
struct snode    *stmt;
...
gendo(stmt) /* generate code for a do - while loop. */
struct snode    *stmt;
...
call_library(lib_name) /* generate a call to a library routine. */
char    *lib_name;
{       SYM     *sp;
        sp = gsearch(lib_name);
        if( sp == 0 )
...
genswitch(stmt) /* generate a linear search switch statement. */
struct snode    *stmt;
{       int             curlab;
...
gencase(stmt) /* generate all cases for a switch statement. */
struct snode    *stmt;
{       while( stmt != 0 )
                {
                if( stmt->s1 != 0 )
...
genxswitch(stmt) /* analyze and generate best switch statement. */
struct snode    *stmt;
{       int     oldbreak;
        oldbreak = breaklab;
        breaklab = nextlabel++;
...
genreturn(stmt) /* generate a return statement. */
struct snode    *stmt;
{       struct amode    *ap;
...
genfunc(stmt) /* generate a function body. */
struct snode    *stmt;
