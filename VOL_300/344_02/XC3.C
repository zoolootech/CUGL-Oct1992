...
int lin_refs = (MAXCOL-30)/5; /* max refs on a printed line */
...
char **p_argv;
{
   ...
   while (--argc)
      if (*(arg = *++argv) == '-')
         ...
         case 'W':
            if (!--argc)
               use_err();
            if ((i = atoi(*++argv)) <= MINCOL || i >= 255)
               use_err();
            maxcol = i;
            lin_refs = (maxcol-30)/5;
            break;
   ...
   linum = 0;
   while (--p_argc && **++p_argv != '-') {
      strcpy(gbl_fil,*p_argv);  
      strcpy(act_fil,*p_argv);
      i = 0;
      while ((gbl_fil[i] != '.') & (gbl_fil[i] != 0x00)) {
         gbl_trc[i] = gbl_fil[i];
         i++;
      }
      gbl_trc[i] = 0x00;
...
void use_err()
{
   ...
   printf("Flags: -c                = Recognize Mapper C keywords\n");
   printf("       -i <opt. path(s)> = Enable file inclusion\n");
   printf("       -l                = Generate listing only\n");
   printf("       -m                = Recognize Microsoft keywords\n");
   printf("       -n                = Allow nested comments\n");
   printf("       -o <outfile>      = Write output to named file\n");
   printf("       -p                = Exclude page headers\n");
...
void prnt_tbl()
{
   int prf_cnt, pti, pref, lin_cnt, refs=0;
      ...
      if (pid_ptr = alpha_vector[pti].alpha_top) {
         do {
            ...
            do {
               if (prf_cnt == MAX_REF) {
                  prf_cnt = 0;
                  ptb_ptr = (struct rf_blk *)ptb_ptr->pnext;
               }
               if (ptb_ptr) {
                  if (pref = ptb_ptr->ref_item[prf_cnt++]) {
                     ...
                     if (o_flg) {
                        if (fprintf(f_lst_fil,"%5u",pref) == ERROR)
                           lst_err();
                     } 
                     else
                        printf("%5u",pref);
   ...
   echo( '\n' );
   if (o_flg)
      if (fprintf(f_lst_fil, "  %6d references", refs) == ERROR)
         lst_err();
...
void prt_hdr()
{
   ...
   if (o_flg) {
      if (p_flg) {
         if (fprintf(f_lst_fil,
         "       'C' Concordance Utility   %-20s      Date:%d/%d/%02d %02d:%02d:%02d",
         gbl_trc, today.da_mon, today.da_day, today.da_year,
         now.ti_hour, now.ti_min, now.ti_sec)
             == ERROR) lst_err();
      } 
      else
         if (fprintf(f_lst_fil,
         "       'C' Concordance Utility   %-20s      Date:%d/%d/%02d %02d:%02d:%02d Page: %d",
         gbl_trc, today.da_mon, today.da_day, today.da_year,
         now.ti_hour, now.ti_min, now.ti_sec, pagno)
             == ERROR) lst_err();
   } 
   else
      printf("XC ... 'C' Concordance Utility   %-20s       Page %d", gbl_trc, pagno);
