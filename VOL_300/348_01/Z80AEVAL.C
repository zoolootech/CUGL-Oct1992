...
/*  Get access to global mailboxes defined in A48.C:			*/
	...
	switch (token.attr & TYPE) {
	    ...
	    case OPR:	if (!(token.attr & UNARY)) { exp_error('E');  break; }
			...
			switch (op) {
			    ...
			    case LENOF:
			    		if (!(u = get_len(token.sval))) exp_error('v'); 
			    		break;
			}

	    case VAL:	
	    case STR:	for (;;) {
			    need_op = TRUE; /* need operator here or else comments */
			    		    /* come back as undefined symbols */
			    op = lex() -> valu;
			    need_op = FALSE;
			    switch (token.attr & TYPE) {
				default:   if (pre != START) unlex();
...
static int inlex = FALSE;

TOKEN *lex()
{
    SCRATCH char c, t, u, *p;
    SCRATCH unsigned b;
    SCRATCH OPCODE *o;
    SCRATCH SYMBOL *s;
    ...
    inlex = TRUE;
    /* trash(); spaces not allowed in expressions */
    if (isalph(c = popc())) {
	...
	if (o = find_operator(token.sval)) {
	    ...
	    if (need_op) {
			...
			token.valu = s -> valu;
			if (s -> attr & PIN_VAL) {
				if (!(s -> attr & PIN))	token.valu += (s -> len - 1);
			}
...
num:		    pops(token.sval);
	...
	case '*':   token.attr = BINARY + UNARY + MULT + OPR;  goto opr1;
...
opr2:		    token.attr = BINARY + RELAT + OPR;  break;
	...
	case '.':   token.attr = UNARY + UOP3 + OPR;
		    token.valu = LENOF;
		    break;

	case ';':   token.attr = SEP;  break;
	
	case ',':   token.attr = SIZE; break;
...
int popc()
{
    ...
    if (eol) return '\n';
    for (;;) {
	if ((c = getc(source)) != EOF && (c &= 0377) == '*' && !quote && !inlex) {
	    do *lptr++ = c;
