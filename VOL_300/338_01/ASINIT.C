...
char *argv[];
{
	...
	puts("AS68, ver 1.02, 68000 assembler, (C) Copyright 1982 Steve Passe\n");

/** parse the command line, setting variables as needed */

    if (argc < 2) {
		puts("Usage: AS68 sourcefile.asm options\n");
		puts("Option\nec    errors to console");
		puts("ee  errors to errorfile  (sourcefile.err)");
		puts("ef  errors to listfile   (sourcefile.ls)");
		puts("lc  list to console");
		puts("lf  list to listfile     (sourcefile.ls)");
		puts("os  object file output   (sourcefile.s19)");
		puts("ox  no object file created");
		puts("t   truncate source code lines in listing");
		puts("w   wrap source code lines in listing");
		exit(0);
    }
...
/* initialize sourcename, errname, listname, and objname */
	...
	*c_ptr = '\0';  			    /* erase it */
	...
	else strcpy(_ext, ".asm");			/* default extension */
	...
	case 's':   strcat(object_name, ".s19");	/* for Motorola	S FILE */
		break;
