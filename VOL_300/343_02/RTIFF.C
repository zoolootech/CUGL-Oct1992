        ...
        short   array[ROWS][COLS];
{
   ...
   file_descriptor = open(image_file_name, O_BINARY | O_RDONLY);
   /*printf("\nRTIFF.C> file desc = %d", file_descriptor);*/
   if(file_descriptor > 0){

   position     = lseek(file_descriptor, image_header.strip_offset, 0);
   position     = seek_to_first_line(file_descriptor, &image_header, il);

   for(i=0; i<(ll-il); i++){
      offset	 = (ie-1)/(8/image_header.bits_per_pixel);
      position	 = lseek(file_descriptor, offset, 1);
      bytes_read = read_line(file_descriptor, array, i, &image_header, ie, le);
      position	 = seek_to_end_of_line(file_descriptor, le, &image_header);
   ...
   closed = close(file_descriptor);
       ...
       *   and puts them into a ROWSxCOLS array of shorts.
       *   The process depends on the number of bits per
       *   pixel used in the file (4 or 8).
       *
       *******************************************************/

read_line(file_descriptor, array, line_number, image_header, ie, le)
