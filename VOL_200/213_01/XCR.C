...
rch :
	...
	{
		/*tab "expansion", less one for immediate next incrementation*/
		if (c == '\t')
			char_count += (7 - ((char_count - 1) & 7));
		if (char_count++ >= LINELENGTH)
		{
			char_count = 1;
			paglin++;
			if (o_flg)
				...
				do
					...
					if (ptb_ptr > MAX_REF)
					{
						if ((pref = ptb_ptr->ref_item[prf_cnt++]) != 0)
						{
							if (o_flg)
							{
								if (fprintf(l_buffer, "%d\t", pref) == ERROR)
									lst_err();
							}
							else
							    printf("%d\t", pref);
...

{
	if (pagno++ != 0)
		if (o_flg)
		{
			if (fprintf(l_buffer, "%c", FF) == ERROR)
				lst_err();
		}
	else
	    printf("%c", FF);
