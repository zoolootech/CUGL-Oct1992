...
char hbuf[], *fontfilename;
{
    ...
    tptr=
      ptr->chinfo=(struct fontspec huge *) halloc(1L,sizeof(struct fontspec));
    ...
    if ((ptr->fontaddr=
             (char huge *) nor_ptr((int far *) halloc(temp,1))) ==
                   (char huge *) NULL) {
        graderror(4,15);
    }
...
char *fontfilename,hbuf[];
{
    ...
    ptr->direction=0;
    ptr->defch=hbuf[8];
    if ((ptr->szinfo=(long huge *) halloc((long) nuchar,sizeof(long)))
                     == (long *) NULL) {
        graderror(4,15);
    }
    if ((tptr=ptr->chinfo=
         (struct fontspec huge *)halloc((long) nuchar,sizeof(struct fontspec)))
    ...
    if ((ptr->fontaddr=halloc(temp,1)) == (char huge *) NULL) {
        graderror(4,15);
    }
    if (dosread(fn,ptr->fontaddr,temp) != temp) {
        graderror(4,14);
