...
extern char *alloca ();
    ...
    synonym for malloc()? Drh 12/30/88
    NOTE: under MSC 5.1, alloca() allocates from STACK, not heap,
          and objects thus obtained can NOT be free()'d.
*/
char any_conflicts;
char *conflicts;
errs **err_table;
...
void resolve_sr_conflict(state, lookaheadnum)
int state;
int lookaheadnum;
{
  register int i;
  ...
  if (errp->nerrs)
    {
     ...
     a permanent errs structure for this state, to record them.  */
      ...
      if (debugflag)
        printf("CONFLICT.C:resolve_sr_conflict request %u bytes\n", i) ;
...
void log_resolution(state, LAno, token, resolution)
