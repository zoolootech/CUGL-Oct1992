...
hier1(lval)  int lval[];  {
  int k,lval2[8], lval3[2], oper, ducon1, ducon2;	/* fas 2.2 */
  k=plnge1(hier3, lval);
  if(match("?")) ducond();				/* fas 2.2 */
...
ducond() {						/* fas 2.2 */
  int flab1, flab2, const, val;

  flab1 = getlabel();	/* label for false branch */
  flab2 = getlabel();	/* label for true exit    */
  testjump(flab1);
  expression(&const,&val);
  jump(flab2);
  postlabel(flab1);
  if(amatch(":",1) ==0) error("colon expected");
  expression(&const,&val);
  postlabel(flab2);
}
