...
delete()                /* delete or replace current instruction */
{
          ...
          case 1:       /* simple instruction */
             demalloc(curr,curr->next);
             break;
          case 2:       /* test */
             prompt("TYPE REPLACEMENT ",1,"",5,0);
             curr->instr = getest();
             print(curr->prev,stdout);
             print(curr,stdout);
             break;
...
char **argv;
{
          ...
          while (p != endof) {
           ...
           fprintf(fptr1, "%d %d %d %d %d %d %d %d \n",
                   9,1,5,4,4,0,2,10);
           print(p,fptr2);
           putc(CPMEOF,fptr1);    putc(CPMEOF,fptr2);
...
match(w,s,maxx)          /* compares string to array element */
char w[], *s[];
int maxx;
{
          int c,i;
