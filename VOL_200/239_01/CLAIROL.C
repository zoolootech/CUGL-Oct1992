...
static void help()
{
      ...
      if (vid_mode == MONOCHROME) p = msg[0]; /* absolute */
      ...
      wputs("^êESC^í  done!");
      for (k = 20; k; k-- ) pause();
      if (kbhit()) ch = keyin( noop ); else ch = '?';
      clrwindow();
}
...
static void inquire()
{
      ...
      while (ch != 27)
      {
            ...
            switch ( ch )
            {
               ...
               case 195:   /* F9?  reverse, or obverse/inverse */
               {
                    if (vid_mode == MONOCHROME)
                    {
