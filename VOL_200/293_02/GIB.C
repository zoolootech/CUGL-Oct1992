...
float ZOOM;
int  buffer[2][6][LINE_SIZE];          /* input buffer */
float fxbuf[5][LINE_SIZE],fybuf[5][LINE_SIZE]; /* X,Y Views floating buffers */ 
float huge fzbuf[LINE_SIZE][LINE_SIZE];        /* Z view floating buffer */
...
{int next,i,j,x,inc;
 ...
 for(i=1,j=n-1;i<n;i++,j--){   /* do for each next line of interpolation */
      for(x=0;x<LINE_SIZE;x++) buffer[line][next][x]=
...
{int z,x,y,i,j,start,stop,inc,line,rzoom,inter;
 ...
 FILE *fxfloat,*fyfloat,*fzfloat,*fn[2];
  ...
  fxfloat=fopen("xdis.dat","wb");    /* X view floating output file */
  fyfloat=fopen("ydis.dat","wb");    /* Y view floating output file */
  for(i=0;i<LINE_SIZE;i++)
   for(j=0;j<LINE_SIZE;j++)fzbuf[i][j=LINE_SIZE; /* clear Z view buffer */
  for(z=0;z<(LASTSLICE-FIRSTSLICE);z++){  /* For each Slice */
      for(i=0;i<2;i++){               /* open next two slice files */
          ...
          fseek(fn[i],(long)CT_SIZE,SEEK_SET); /* skip header block */
      } 
  ...
  for(i=0;i<LINE_SIZE;i++)fwrite(fzbuf[i],1,1024,fzfloat);
  fclose(fzfloat);
}
...
{float gx,gy,gz;
 unsigned char gxint;
  ...
  gxint=(LINE_SIZE-1)*gx+0.5;      /*scale gx by LINE_SIZE and round */
  return(gxint);
}
...
{int i;
  ...
  if(fxbuf[line][0]==LINE_SIZE)lineg[0]=lined[0]=0;
  else{
      lined[0]=LAST_VAL-fxbuf[line][0]; /*distance shade */
      lineg[0]=grad(fxbuf[line][0],fxbuf[line][1],fxbuf[prevline][0],
                       fxbuf[succline][0],1,z_factor);
  }
  /* do rest of pixels inside line */
  for(i=1;i<LAST_VAL;i++) if(fxbuf[line][i]==LINE_SIZE)lineg[i]=lined[i]=0;
  else{
      lined[i]=LAST_VAL-fxbuf[line][i]; /*distance shade */
      lineg[i]=grad(fxbuf[line][i-1],fxbuf[line][i+1],fxbuf[prevline][i],
                       fxbuf[succline][i],2,z_factor);
  }
  /* do last pixel in line */
  if(fxbuf[line][LAST_VAL]==LINE_SIZE)lineg[LAST_VAL]=lined[LAST_VAL]=0;
  else{
      lined[LAST_VAL]=LAST_VAL-fxbuf[line][LAST_VAL]; /*distance shade */
      lineg[LAST_VAL]=grad(fxbuf[line][LAST_VAL],fxbuf[line][254],fxbuf[prevline][LAST_VAL],
                       fxbuf[succline][LAST_VAL],1,z_factor);
  }
  fwrite(lineg,1,LINE_SIZE,fg); /* write to output files */
  fwrite(lined,1,LINE_SIZE,fd);
}
...
{FILE *fg,*fd,*ffloat;
 int z,i,j,k,midline;
 char lined[LINE_SIZE],lineg[LINE_SIZE]; /* gradient and distance value buffers */
...
main()
{
 ...
 doviews("zdis.dat","gbo.out","dbo.out",LINE_SIZE);
 printf("doing right lateral (X) views\n");
 doviews("xdis.dat","grl.out","drl.out",NLINES);
 printf("doing rear (Y) views\n");
 doviews("ydis.dat","gre.out","dre.out",NLINES);
