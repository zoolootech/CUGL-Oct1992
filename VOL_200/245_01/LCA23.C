...
# define TS	     8	/* distinct sums w/totalistic rule	*/
# define DS	    32  /* (number of distinct neighborhoods)/4	*/
# define KK	     2  /* number of states per cell		*/
# define NX         14	/* number of sample rules		*/

char xrule[NX][KK][KK][KK][KK][KK];

char ixrule[NX][DS]=

	"00AC324A8D30329A99A108A46C9FDCB7",	/* cycles on dgl bgrnd	*/
	"088686698669337E8669699E699E9EE7",	/* class iv - try vnts	*/
	"088E8CC78EAEE5888CC7E67FE7717118",	/* very complex gliders	*/
	"088E8EE78EE7E7798EE7E779E7797996",	/* Extrmly good varnts	*/
	"088071C431F94830C78A435E015198DC",

	"1008088608868669088686698669699E",	/* dominant triangles	*/
	"11000100000010110111001011001100",	/* flotsam & jetsam	*/
	"28CA858DC0500D0512AAA0BACBBB94B3",
	"381E3AFA19D7CECBCE5867F7206F7B6E",	/* mottled		*/
	"4A1ADAD80AAC3E8E8AEE4D138308E0FB",	/* crossed gliders	*/

	"88EF636569ABE6994C073A1FF3AC5EFE",	/*			*/
	"8A0FFA23E1FF8558675AFF0B53AD9FCB",	/* dislocatns & regns	*/
	"A3F73FA3A81F3F4AE4280DED0E317F1B",
	"D2BF4202000000005CEDB7C000000000"	/* dislocation		*/

	;

char  xx[4], rule[DS+1], ascrule[KK][KK][KK][KK][KK][KK][KK];
int   binrule[KK][KK][KK][KK][KK][KK][KK], arule[DS], arr1[AL], arr2[AL];
char  trule[TS]="00000000";

main()
{
int  i, j, i0, i1, i2, i3, i4, i5, i6;
int  more = 'r';
char a, b, c;
...
for (j=0; j<DS; j++) {
  ...
  if (i0==KK) {i0=0; };
	...
	case 't':					/* totalistic rule */
	    ...
	    for (i5=0; i5<KK; i5++) {
	    for (i6=0; i6<KK; i6++) {
	    ascrule[i0][i1][i2][i3][i4][i5][i6]=trule[i0+i1+i2+i3+i4+i5+i6];
	    };};};};};};};
	    videocursor(0,4,0);
	    rmenu();
	    xmenu(totonu(0));
	...
	case 'x':					/* random rule */
	    ...
	    for (i6=0; i6<KK; i6++) {
	      if (i4 == 0) i=rand();
	      ascrule[i0][i1][i2][i3][i4][i5][i6]='0'+i%2;
	      i/=2;
	    };};};};};};};
	    rmenu();
	    break;
	...
	case 'Y':					/* symmetrize rule */
	    ...
	    for (i5=0; i5<KK; i5++) {
	    for (i6=0; i6<KK; i6++) {
	    ascrule[i6][i5][i4][i3][i2][i1][i0]=ascrule[i0][i1][i2][i3][i4][i5][i6];      
	    };};};};};};};
	    break;
	case 'B':					/* begin barrier */
	    a=kbdin();
	    b=kbdin();
	    ascrule[0][0][0][0][0][a-'0'][b-'0']=a;
	    ascrule[0][0][1][0][0][a-'0'][b-'0']=a;
	    ascrule[0][0][2][0][0][a-'0'][b-'0']=a;
	    rmenu();
	    break;
	case 'E':					/* end barrier */
	    a=kbdin();
	    b=kbdin();
	    ascrule[0][0][0][0][a-'0'][b-'0'][0]=b;
	    ascrule[0][0][0][0][a-'0'][b-'0'][1]=b;
	    ascrule[0][0][0][0][a-'0'][b-'0'][2]=b;
	    rmenu();
	    break;
...
i=6; i0=0; i1=0; i2=0; i3=0; i4=0;
    while (0<1) {
        ...
        switch (c) {
        case '0':					/* state */
	    ascrule[i0][i1][i2][i3][i4][0][0]='0';
	    ascrule[i0][i1][i2][i3][i4][0][1]='0';
	    ascrule[i0][i1][i2][i3][i4][1][0]='0';
	    ascrule[i0][i1][i2][i3][i4][1][1]='0';
    	    i4++;
	    if (i4==KK) {i4=0; i3++;};
	    if (i3==KK) {i3=0; i2++;};
	    if (i2==KK) {i2=0; i1++;};
...
int i0, i1, i2, i3, i4, i5, i6, i, j;
  ...
  for (i5=0; i5<KK; i5++) {
  for (i6=0; i6<KK; i6++) {
    binrule[i0][i1][i2][i3][i4][i5][i6]=ascrule[i0][i1][i2][i3][i4][i5][i6]-'0';
    };};};};};};};
  for (j=8; j<200; j++) videodot(j,AL-1,2);
  for (j=8; j<200; j++) {				/* evolve for 192 generations */
    arr2[0]=binrule[arr1[AL-3]][arr1[AL-2]][arr1[AL-1]][arr1[0]][arr1[1]][arr1[2]][arr1[3]];
    arr2[1]=binrule[arr1[AL-2]][arr1[AL-1]][arr1[0]][arr1[1]][arr1[2]][arr1[3]][arr1[4]];
    arr2[2]=binrule[arr1[AL-1]][arr1[0]][arr1[1]][arr1[2]][arr1[3]][arr1[4]][arr1[5]];
    for (i=3; i<AL-3; i++) {
      arr2[i]=binrule[arr1[i-3]][arr1[i-2]][arr1[i-1]][arr1[i]][arr1[i+1]][arr1[i+2]][arr1[i+3]]; };
    arr2[AL-3]=binrule[arr1[AL-6]][arr1[AL-5]][arr1[AL-4]][arr1[AL-3]][arr1[AL-2]][arr1[AL-1]][arr1[0]];
    arr2[AL-2]=binrule[arr1[AL-5]][arr1[AL-4]][arr1[AL-3]][arr1[AL-2]][arr1[AL-1]][arr1[0]][arr1[1]];
    arr2[AL-1]=binrule[arr1[AL-4]][arr1[AL-3]][arr1[AL-2]][arr1[AL-1]][arr1[0]][arr1[1]][arr1[2]];
...
int istot() {int i0, i1, i2, i3, i4, i5, i6, l;
    l=1;
    trule[0]=ascrule[0][0][0][0][0][0][0];
    trule[1]=ascrule[1][0][0][0][0][0][0];
    trule[2]=ascrule[1][1][0][0][0][0][0];
    trule[3]=ascrule[1][1][1][0][0][0][0];
    trule[4]=ascrule[1][1][1][1][0][0][0];
    trule[5]=ascrule[1][1][1][1][1][0][0];
    trule[6]=ascrule[1][1][1][1][1][1][0];
    trule[7]=ascrule[1][1][1][1][1][1][1];
    for (i0=0; i0<KK; i0++) {
    for (i1=0; i1<KK; i1++) {
    for (i2=0; i2<KK; i2++) {
    for (i3=0; i3<KK; i3++) {
    ...
    for (i5=0; i5<KK; i5++) {
    for (i6=0; i6<KK; i6++) {
    if (ascrule[i0][i1][i2][i3][i4][i5][i6]!=trule[i0+i1+i2+i3+i4+i5+i6]) l=0;      
    };};};};};};};
    return l; }
...
scrrul() {char i; int i0, i1, i2, i3, i4;
  for (i0=0; i0<KK; i0++) {
  for (i1=0; i1<KK; i1++) {
  for (i2=0; i2<KK; i2++) {
  for (i3=0; i3<KK; i3++) {
  for (i4=0; i4<KK; i4++) {
  i='0'+(ascrule[i0][i1][i2][i3][i4][0][0]-'0')+
      2*(ascrule[i0][i1][i2][i3][i4][0][1]-'0')+
      4*(ascrule[i0][i1][i2][i3][i4][1][0]-'0')+
      8*(ascrule[i0][i1][i2][i3][i4][1][1]-'0');
  if (i>'9') i=i+('A'-':');
  videoputc(i,1);      
  };};};};}; }
