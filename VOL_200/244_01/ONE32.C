...
/* version for totalistic (3,2), first generation */
...
char arry[3][3][3][3][3];
int  rule[NS] = 0,1,0,0,1,2,1,2,3,3,1;
int  nc, nl;
...
do {
printf("  a=still  b=slowglider  c=fastglider  0,1,2=constant  q=quit\015");
c=kbdin();
if (c=='q') break;
switch (c) {
case 'a':
...
if ((arry[i0][i1][i2][i3][i4]&0x5F)=='Y')
 {for (m=0; m<3; m++) arry[i1][i2][i3][i4][m]|=0x20;};
};};};};};
} while (pass3()!=0);
}
...
for (i4=0; i4<3; i4++) {
if ((arry[i0][i1][i2][i3][i4]&0x5F)=='Y')
 {for (m=0; m<3; m++) arry[m][i0][i1][i2][i3]|=0x20;};
};};};};};
} while(pass3()!=0);
}
