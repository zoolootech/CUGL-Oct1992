/*
**	file:		reader.c
**	purpose:	Read the input specification
*/
#include <stdio.h>
#include <stdlib.h>
#include <ctype.h>
...
void parse_token_decl (what_is, what_is_not)
...
/*      if (lineno != start_lineno)
    ...
    {
      if (symval->class == (char) what_is_not)
        fatals("symbol %s redefined", symval->tag);
      symval->class = (char) what_is;
      if (what_is == SNTERM)
        symval->value = nvars++;
...
void copy_guard(rule, stack_offset)
symbol_list *rule;
int stack_offset;
{
  register int c;
...
void copy_action(rule, stack_offset)
symbol_list *rule;
int stack_offset;
{
  register int c;
...
int read_signed_integer (stream)
     FILE *stream;
{
