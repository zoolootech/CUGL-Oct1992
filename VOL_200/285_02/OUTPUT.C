...
/* 
    Microsoft v5.1 variations:
        malloc() gets 65516 bytes MAX in one object,
        so MAXTABLE is constrained to 65516 / sizeof( short int))
        = 32758 or less.
*/
#define MAXTABLE 32700

#define DOSTRACK 4608           /* bytes per physical track in vanilla DOS */
...
void output_token_translations()
{
    ...
    if (translations)
    {
        ...
        for (i = 1; i <= max_user_token_number; i++)
        {
            ...
            fprintf(ftable, "%4d", token_translations[i]);
        }
...
void output_rule_data()
{
    ...
    for (i = 1; i < nrules; i++)
    {
        ...
        fprintf(ftable, "%4d", rrhs[i + 1] - rrhs[i] - 1);
    }
...
/*
    DrH 1/6/89
        test for "#line" or "#    line" directive in parser file.
*/

int has_linexp( register char *s)
{
 register char *t ;
    ...
    if (t = strchr( s, '#'))
    {
        ...
        while ((*t) && isspace( *t)) ++t ;    /* skip intervening whitespace */

        if ( *t != 'l' )                      /* quick exit */
            return FALSE ;

        return (( strncmp( t, "line", 4)) ? FALSE : TRUE ) ;
    }
    return FALSE ;
}
...
void output_parser()
{
    register int c, bytesread;
    char linebuf[256], *sptr ;
#ifdef DONTDEF
    FILE *fpars;
#else
...
/*
DrH 1/6/89
    Fixed '#line' detector so that it (A.) actually works, and
    (B.) doesn't insert an EOF in the middle of the output file,
    just like the old PD-YACC.

    NOTE: file activity is now line and block-oriented since the
            operation is largely file copying.
*/
    while (sptr = fgets( &linebuf[0], 255, fpars))
    {
        if (nolinesflag)
        {
            if ( has_linexp( &linebuf[0]))
                continue ;
        }

        if (sptr = strchr( &linebuf[0], '$'))
        {
            fputs( (sptr+1), ftable) ;              /* move past the '$' */
                                                    /* and write the rest */
#ifdef DONTDEF
            fprintf(ftable, "#include \"%s\"\n", actfile);
#else
            /* JF don't #include the action file.  Stuff it right in. */
            /* DrH  1/6/89  block file I/O */

            rewind(faction);

            sptr = NEW2(DOSTRACK, char) ;        /* 4608 = DOS bytes per track */

            while ((bytesread = fread( sptr, 1, DOSTRACK, faction)) > 0)
                (void) fwrite( sptr, 1, bytesread, ftable) ;

            FREE( sptr) ;
#endif
        }
        else
            fputs( &linebuf[0], ftable) ;
...
/* DrH 1/6/89 block file I/O */
    ...
    while ((bytesread = fread( buff, 1, DOSTRACK, finput)) > 0)
        (void) fwrite( buff, 1, bytesread, ftable) ;
