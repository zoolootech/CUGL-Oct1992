...
**  typeafil.c
**  26 oct 86, dco
**  21 feb 88, dco (account for msc 5 changes in strerror fn)
**  4/23/88, removed signal code, doesn't belong in service routines!
**           but left in Control_C_Flag (initialized to 0) for compatibility
**			 with old programs that test the flag. - d.c.oshel
*/
...
int Control_C_Flag = 0;  /* global used to inform caller that exit was by Control-C */
...
static void muf( p ) char *p;
{
     ...
     */
     ...
     if (lines >= deep)
     {
        ...
        if ((i = keyin( screenwait )) == ESC) yo_heave_ho();  /* FLOW STATEMENT #1 */
...
static void showscreen()
{
    ...
    if (!contin)  /* end of file condition */
    {
        ...
        while (kbhit()) getch();
        review = (lines = keyin(screenwait)) == PGUP;
    }
...
{
    static long filepos;    /* where each screen page starts in the file */
...
zoo:    thurb();
        ...
        setsynch(1); 
        wprintf( "Might have been: ^1%s^", strerror( t_errnum )); 
        setsynch(0); 
        wprintf("\n^1--any key continues--^"); 
