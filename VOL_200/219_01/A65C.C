...
char **argv;
{
    ...
    printf("65C02 Cross-Assembler (Portable) Ver 0.1\n");
...
{
    SCRATCH unsigned opcode, operand, opernd2;
    unsigned do_args(), expr();
    void do_label(), error();

    opcode = opcod -> valu;  bytes = BIGINST;
    do_label();  operand = do_args();
    switch (opcod -> attr) {
	case RMBOP: if (argattr != ARGNUM) error('A');
		    if (operand > 0xff) { error('V');  operand = 0; }
		    bytes = 2;  break;

	case CPXY:  if (argattr & ARGIMM) goto do_immediate;
		    opcode += 0x04;

	case TSBOP: if (argattr != ARGNUM) { error('A');  return; }
		    goto do_zero_page;

	case INHOP: if (argattr) error('T');
	    	    bytes = 1;  break;

	case JUMP:  if (argattr == (ARGX + ARGIND + ARGNUM)) {
			opcode = 0x7c;  break;
		    }
	...
	case BBOP:  if (argattr != ARGNUM) { error('A');  return; }
		    if ((opernd2 = expr() - (pc + 3)) > 0x007f &&
			opernd2 < 0xff80) {
			error('B');  opernd2 = 0xfffd;
		    }
		    operand = (low(opernd2) << 8) + low(operand);
		    break;

	case RELBR: if (argattr != ARGNUM) { error('A');  return; }
		    operand -= pc + 2;  bytes = 2;
	...
	case BIT:   if (argattr & ARGIMM) goto do_immediate;
		    opcode = 0x24;  goto do_inc_op;
...
do_immediate:	    if (operand > 0x00ff && operand < 0xff80) {
	...
	case INCOP: if (argattr & ARGA) {
			opcode = opcode == 0xc6 ? 0x3a : 0x1a;
			bytes = 1;  break;
		    }
do_inc_op:	    if ((argattr & ~ARGX) != ARGNUM) { error('A');  return; }
do_indexed_x:	    if (argattr & ARGX) opcode += 0x10;
do_zero_page:	    if (!forwd && operand <= 0x00ff) bytes = 2;
		    else opcode += 0x08;
		    break;

	case STZ:   if ((argattr & ~ARGX) != ARGNUM) { error('A');  return; }
		    if (!forwd && operand <= 0x00ff) {
			if (argattr & ARGX) opcode = 0x74;
			bytes = 2;
		    }
		    else opcode = (argattr & ARGX) ? 0x9e : 0x9c;
		    break;
    }
