/*
**	file:		main.c
**	purpose:	Tool to generate fast lexical analyzers
...
/*
**	function:	usage
**	purpose:	Show syntax of message
*/
void
usage(void)
{
fprintf(stderr,"FLEX (Version 1.04b)\t\t\t\t12/30/1989\n");
fprintf(stderr,
"Usage: flex [ -dfirstvFILT -c[efmF] -Sskeleton_file ] <filename.L>\n");
fprintf(stderr,"\t-d = Scanner runs in DEBUG mode.\n");
fprintf(stderr,"\t-f = Do not compress scanner tables.\n");
fprintf(stderr,"\t-i = Generate case-insensitive scanner.\n");
fprintf(stderr,"\t-r = Use the REJECT action.\n");
fprintf(stderr,"\t-s = Suppress default rule.\n");
fprintf(stderr,"\t-t = Identical to LEX's option.\n");
fprintf(stderr,"\t-v = Verbose. Print summary statistics.\n");
fprintf(stderr,"\t-F = Fast scanner table representation.\n");
fprintf(stderr,"\t-I = Generate an interactive scanner.\n");
fprintf(stderr,"\t-L = Do not generate #line directives.\n");
fprintf(stderr,"\t-T = Run FLEX in trace mode.\n");
fprintf(stderr,"\t-c[efmF] = Controls degree of table compresion.\n");
fprintf(stderr,"\t-Sskeleton_file = Overrides default skeleton.\n");
fprintf(stderr,"\t<filename.L> = The FLEX grammar file to process.\n");
	return;
};					/* usage */
...
char **argv;
{
    if (argc == 1)			/* Only name inputed? */
    {					/* Yes, */
    	usage();			/* Give me syntax */
    	exit(1);			/* Good bye! */
    };					/* endif */
...

{
    ...
    char *arg, *skelname = NULL ;
    char *flexenv = "" ;
    char *skelpath = "" ;
    /*    char *gettime(), clower(), *mktemp(); */
...
get_next_arg: /* used by -c and -S flags in lieu of a "continue 2" control */
    ...
    if ( ! use_stdout )
    {
        ...
        if ( prev_stdout == NULL )
            flexerror( "could not create lexyy.c" );
    }
    ...
    {
        if (!(flexenv = getenv("FLEXENV")))
            lerrsf( "can't open skeleton file %s", skelname );
        else
           if (!(skelpath = malloc( strlen(skelname) + strlen(flexenv) + 2)))
               lerrsf("%s: out of memory.", "flexinit()") ;

        sprintf( skelpath,"%s\\%s", flexenv, skelname) ;

        if ( (skelfile = fopen( skelpath, "r" )) == NULL )
           lerrsf( "can't open skeleton file %s", skelpath ); 
        else
           free( skelpath) ;
    }

    action_file_name = mktemp( "flXXXXXX" ) ;
...

{
    ...
    dss = allocate_intptr_array( current_max_dfas );
